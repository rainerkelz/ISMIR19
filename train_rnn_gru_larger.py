...
########################################
BD_FACTOR = 2 if BIDIRECTIONAL else 1
HIDDEN_SHAPE = (N_LAYERS * BD_FACTOR, 1, N_HIDDEN)
...
def train(logger, device, recurrent_model, output_model, optimizer, loader, global_step):
    ...
    for batch in loader:
        h_0 = torch.zeros(*HIDDEN_SHAPE).to(device)
        x = batch['x'].to(device)
        y = batch['y'].to(device)
...
def main():
    ...
    # optimizer = torch.optim.Adam(parameters, lr=1e-2, betas=(0.9, 0.999))
    ...
    log_dir = 'runs/rnn_gru_larger_maps_spec2labels_swd'
